...
pub enum SocketEvent {
    ...
    CONNECTED = ZMQ_EVENT_CONNECTED as isize,
    CONNECT_DELAYED = ZMQ_EVENT_CONNECT_DELAYED as isize,
    CONNECT_RETRIED = ZMQ_EVENT_CONNECT_RETRIED as isize,
    LISTENING = ZMQ_EVENT_LISTENING as isize,
    BIND_FAILED = ZMQ_EVENT_BIND_FAILED as isize,
    ACCEPTED = ZMQ_EVENT_ACCEPTED as isize,
    ACCEPT_FAILED = ZMQ_EVENT_ACCEPT_FAILED as isize,
    CLOSED = ZMQ_EVENT_CLOSED as isize,
    CLOSE_FAILED = ZMQ_EVENT_CLOSE_FAILED as isize,
    DISCONNECTED = ZMQ_EVENT_DISCONNECTED as isize,
    MONITOR_STOPPED = ZMQ_EVENT_MONITOR_STOPPED as isize,
    HANDSHAKE_FAILED_NO_DETAIL = ZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL as isize,
    HANDSHAKE_SUCCEEDED = ZMQ_EVENT_HANDSHAKE_SUCCEEDED as isize,
    HANDSHAKE_FAILED_PROTOCOL = ZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL as isize,
    HANDSHAKE_FAILED_AUTH = ZMQ_EVENT_HANDSHAKE_FAILED_AUTH as isize,
    ALL = ZMQ_EVENT_ALL as isize,
}
...
impl SocketEvent {
    ...
    pub fn from_raw(raw: u16) -> SocketEvent {
        use SocketEvent::*;
        match u32::from(raw) {
            ZMQ_EVENT_CONNECTED => CONNECTED,
            ZMQ_EVENT_CONNECT_DELAYED => CONNECT_DELAYED,
            ZMQ_EVENT_CONNECT_RETRIED => CONNECT_RETRIED,
            ZMQ_EVENT_LISTENING => LISTENING,
            ZMQ_EVENT_BIND_FAILED => BIND_FAILED,
            ZMQ_EVENT_ACCEPTED => ACCEPTED,
            ZMQ_EVENT_ACCEPT_FAILED => ACCEPT_FAILED,
            ZMQ_EVENT_CLOSED => CLOSED,
            ZMQ_EVENT_CLOSE_FAILED => CLOSE_FAILED,
            ZMQ_EVENT_DISCONNECTED => DISCONNECTED,
            ZMQ_EVENT_MONITOR_STOPPED => MONITOR_STOPPED,
            ZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL => HANDSHAKE_FAILED_NO_DETAIL,
            ZMQ_EVENT_HANDSHAKE_SUCCEEDED => HANDSHAKE_SUCCEEDED,
            ZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL => HANDSHAKE_FAILED_PROTOCOL,
            ZMQ_EVENT_HANDSHAKE_FAILED_AUTH => HANDSHAKE_FAILED_AUTH,
            ZMQ_EVENT_ALL => ALL,
...
impl Error {
    ...
    pub fn message(self) -> &'static str {
        unsafe {
            let s = zmq_strerror(self.to_raw());
...
impl RawContext {
    fn term(&self) -> Result<()> {
        zmq_try!(unsafe { zmq_ctx_term(self.ctx) });
        Ok(())
    }
}
...
impl Context {
    ...
    pub fn get_io_threads(&self) -> Result<i32> {
        let rc =
            zmq_try!(unsafe { zmq_ctx_get(self.raw.ctx, ZMQ_IO_THREADS as _) });
        Ok(rc as i32)
    }
    ...
    pub fn set_io_threads(&self, value: i32) -> Result<()> {
        zmq_try!(unsafe {
            zmq_ctx_set(self.raw.ctx, ZMQ_IO_THREADS as _, value as i32)
        });
        Ok(())
    }
    ...
    pub fn socket(&self, socket_type: SocketType) -> Result<Socket> {
        let sock = unsafe { zmq_socket(self.raw.ctx, socket_type.to_raw()) };
...
impl Drop for Socket {
    fn drop(&mut self) {
        if self.owned && unsafe { zmq_close(self.sock) } == -1 {
    ...
    ( $(#[$meta:meta])*
      pub $getter:ident => $constant_name:ident as $ty:ty
        ...
        pub fn $getter(&self) -> Result<$ty> {
            <$ty as sockopt::Getter>::get(self.sock, $constant_name as c_int)
        }
    };
}
    ...
    ( $(#[$meta:meta])*
      pub $setter:ident => $constant_name:ident as $ty:ty
        ...
        pub fn $setter(&self, value: $ty) -> Result<()> {
            <$ty as sockopt::Setter>::set(self.sock, $constant_name as c_int, value)
        }
    };
}
    ...
    T: Into<Message>,
{
    fn send(self, socket: &Socket, flags: i32) -> Result<()> {
        ...
        zmq_try!(unsafe { zmq_msg_send(msg_ptr(&mut msg), socket.sock, flags as c_int) });
        Ok(())
    }
}
...
impl Socket {
    ...
    pub fn bind(&self, endpoint: &str) -> Result<()> {
        ...
        zmq_try!(unsafe { zmq_bind(self.sock, c_str.as_ptr()) });
        Ok(())
    }
    ...
    pub fn unbind(&self, endpoint: &str) -> Result<()> {
        let c_str = ffi::CString::new(endpoint.as_bytes()).unwrap();
        zmq_try!(unsafe { zmq_unbind(self.sock, c_str.as_ptr()) });
        Ok(())
    }
    ...
    pub fn connect(&self, endpoint: &str) -> Result<()> {
        let c_str = ffi::CString::new(endpoint.as_bytes()).unwrap();
        zmq_try!(unsafe { zmq_connect(self.sock, c_str.as_ptr()) });
        Ok(())
    }
    ...
    pub fn disconnect(&self, endpoint: &str) -> Result<()> {
        let c_str = ffi::CString::new(endpoint.as_bytes()).unwrap();
        zmq_try!(unsafe { zmq_disconnect(self.sock, c_str.as_ptr()) });
        Ok(())
    }
    ...
    pub fn monitor(&self, monitor_endpoint: &str, events: i32) -> Result<()> {
        ...
        zmq_try!(unsafe {
            zmq_socket_monitor(self.sock, c_str.as_ptr(), events as c_int)
        });
        Ok(())
    }
    ...
    pub fn recv(&self, msg: &mut Message, flags: i32) -> Result<()> {
        zmq_try!(unsafe { zmq_msg_recv(msg_ptr(msg), self.sock, flags as c_int) });
        Ok(())
    }
    ...
    pub fn recv_into(&self, bytes: &mut [u8], flags: i32) -> Result<usize> {
        ...
        let rc = zmq_try!(unsafe {
            zmq_recv(self.sock, bytes_ptr, bytes.len(), flags as c_int)
        });
        Ok(rc as usize)
    }
    ...
    pub fn get_socket_type(&self) -> Result<SocketType> {
        sockopt::get(self.sock, ZMQ_TYPE as c_int).map(SocketType::from_raw)
    }

    /// Return true if there are more frames of a multipart message to receive.
    pub fn get_rcvmore(&self) -> Result<bool> {
        sockopt::get(self.sock, ZMQ_RCVMORE as c_int).map(|o: i64| o == 1i64)
    }
    ...
    pub fn get_identity(&self) -> Result<Vec<u8>> {
        ...
        sockopt::get_bytes(self.sock, ZMQ_ROUTING_ID as c_int, 255)
    }
    ...
    pub fn get_socks_proxy(&self) -> Result<result::Result<String, Vec<u8>>> {
        ...
        sockopt::get_string(self.sock, ZMQ_SOCKS_PROXY as c_int, 255, true)
    }

    pub fn get_mechanism(&self) -> Result<Mechanism> {
        sockopt::get(self.sock, ZMQ_MECHANISM as c_int).map(|mech| match mech {
            ZMQ_NULL => Mechanism::ZMQ_NULL,
            ZMQ_PLAIN => Mechanism::ZMQ_PLAIN,
            ZMQ_CURVE => Mechanism::ZMQ_CURVE,
            ZMQ_GSSAPI => Mechanism::ZMQ_GSSAPI,
    ...
    pub fn get_plain_username(&self) -> Result<result::Result<String, Vec<u8>>> {
        // 255 = arbitrary size
        sockopt::get_string(self.sock, ZMQ_PLAIN_USERNAME as c_int, 255, true)
    }

    pub fn get_plain_password(&self) -> Result<result::Result<String, Vec<u8>>> {
        // 256 = arbitrary size based on std crypto key size
        sockopt::get_string(self.sock, ZMQ_PLAIN_PASSWORD as c_int, 256, true)
    }

    pub fn get_zap_domain(&self) -> Result<result::Result<String, Vec<u8>>> {
        // 255 = arbitrary size
        sockopt::get_string(self.sock, ZMQ_ZAP_DOMAIN as c_int, 255, true)
    }
    ...
    pub fn get_curve_publickey(&self) -> Result<Vec<u8>> {
        sockopt::get_bytes(self.sock, ZMQ_CURVE_PUBLICKEY as c_int, 32)
    }
    ...
    pub fn get_curve_secretkey(&self) -> Result<Vec<u8>> {
        sockopt::get_bytes(self.sock, ZMQ_CURVE_SECRETKEY as c_int, 32)
    }
    ...
    pub fn get_curve_serverkey(&self) -> Result<Vec<u8>> {
        ...
        sockopt::get_bytes(self.sock, ZMQ_CURVE_SERVERKEY as c_int, 32)
    }

    pub fn get_gssapi_principal(&self) -> Result<result::Result<String, Vec<u8>>> {
        // 260 = best guess of max length based on docs.
        sockopt::get_string(self.sock, ZMQ_GSSAPI_PRINCIPAL as c_int, 260, true)
    }
    ...
    pub fn get_gssapi_service_principal(&self) -> Result<result::Result<String, Vec<u8>>> {
        ...
        sockopt::get_string(
            self.sock,
            ZMQ_GSSAPI_SERVICE_PRINCIPAL as c_int,
            260,
            true,
        )
    }
...
pub fn poll(items: &mut [PollItem], timeout: i64) -> Result<i32> {
    let rc = zmq_try!(unsafe {
        zmq_poll(
            items.as_mut_ptr() as *mut zmq_pollitem_t,
            items.len() as c_int,
            timeout as c_long,
        )
    });
...
pub fn proxy(frontend: &Socket, backend: &Socket) -> Result<()> {
    zmq_try!(unsafe { zmq_proxy(frontend.sock, backend.sock, ptr::null_mut()) });
    Ok(())
}
...
pub fn proxy_with_capture(
    ...
    zmq_try!(unsafe { zmq_proxy(frontend.sock, backend.sock, capture.sock) });
    Ok(())
}
...
pub fn proxy_steerable(
    ...
    zmq_try!(unsafe {
        zmq_proxy_steerable(frontend.sock, backend.sock, ptr::null_mut(), control.sock)
    });
    Ok(())
}
...
pub fn proxy_steerable_with_capture(
    ...
    zmq_try!(unsafe {
        zmq_proxy_steerable(frontend.sock, backend.sock, capture.sock, control.sock)
    });
    Ok(())
}
...
pub fn has(capability: &str) -> Option<bool> {
    ...
    unsafe { Some(zmq_has(c_str.as_ptr()) == 1) }
}
...
pub fn z85_decode(data: &str) -> result::Result<Vec<u8>, DecodeError> {
    ...
    unsafe {
        zmq_z85_decode(dest.as_mut_ptr(), c_str.into_raw());
    }
